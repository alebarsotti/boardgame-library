...
Image curation follows the same build-time workflow through `data/image-overrides.json`, keyed by game id.

The file accepts either:

- a plain string value
- or an object with an `imageUrl` field

Examples:

```json
{
  "267814": "https://example.com/custom-cover.jpg",
  "171623": {
    "imageUrl": "assets/custom-covers/marco-polo.jpg"
  }
}
```
...
- absolute remote URLs such as `https://...`
- relative local paths such as `assets/custom-covers/marco-polo.jpg`

Just like name overrides, there is no runtime editing UI. Rebuild the dataset after updating the file:
